...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    fetch("http://localhost:8080/api/login", {
      ...
      .then((data) => {
        if (data["jwt-token"]) {
          console.log("Login successful, processing token..."); // 確認成功
          localStorage.setItem("jwt-token", data["jwt-token"]); // 將 jwt-token 存入 Local Storage

          // 使用 jwt-decode 函式庫解析 JWT token
          const decodedToken = jwt_decode(data["jwt-token"]);
          console.log("Decoded JWT Token:", decodedToken);

          // 根據解析出來的 status 決定跳轉的頁面
          if (decodedToken.status) {
            switch (decodedToken.status) {
              case "user":
                window.location.href = "index.html"; // 如果 status 是 "user"，跳轉到 index.html
                break;
              case "admin":
                window.location.href = "backuser.html"; // 如果 status 是 "admin"，跳轉到 backuser.html
                break;
              default:
                // 可以選擇跳轉到一個通用錯誤頁面或是保持在當前頁面並顯示錯誤信息
                console.error("Unknown status, cannot redirect.");
                break;
            }
          } else {
            throw new Error("Token does not contain status");
          }
        } else {
